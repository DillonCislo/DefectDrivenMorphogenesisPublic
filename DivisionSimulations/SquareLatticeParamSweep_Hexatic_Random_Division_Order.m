...
%   lattice with modest hexatic order, cells are divided in a random order
%   through a user specified number of divisions. Cell division axes are
%   drawn from a von Mises distribution. The concentration of the von Mises
%   distribution is the parameter being swept
%
%   by Dillon Cislo 2022/06/19
%========================================================================
...
%     'Noisy_Hexatic_SNR18_Random_Divisions_Param_Sweep_Results_202206198');
saveDir = fullfile(projectDir, ... % Make the data yourself
    ['Noisy_Hexatic_SNR18_Random_Divisions_Param_Sweep_Results_' date]);
...
parfor simID = 1:numSim
    ...
    for nwIDx = 1:numDivWaves
        ...
        if randomDiv
            fprintf('Randomizing division order... \n');
            divOrder = divOrder(randperm(numel(divOrder)));
        end
        ...
        for didx = 1:numel(divOrder)
            ...
            if verbose
                try
                    ...
                    fprintf('Simulating the %d/%d division for current wave\n', ...
                        didx, numel(divOrder));
                end
            end
    ...
    warning('on', 'all');
        ...
        warning('Failed simulation %d for k = %d\n', kSimID, allK(kIDx));
